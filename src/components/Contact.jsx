...
/* ----------------------------------------------------------------------------
   ...
   Creates an animated cyberpunk grid with pulsating lines.
------------------------------------------------------------------------------*/
const LINE_COUNT = 30;
const LINE_SPACING = 100 / LINE_COUNT; // percentage for each line step

function GridBackground() {
  // Build array of line indices once
  const lines = Array.from({ length: LINE_COUNT }, (_, i) => i);

  // Common animation settings for lines
  const lineMotion = {
    animate: {
      opacity: [0.2, 0.5, 0.2],
      scale: [1, 1.5, 1],
        ...
        {lines.map((i) => (
          <motion.div
            ...
            style={{
              left: `${i * LINE_SPACING}%`,
              background:
                'linear-gradient(180deg, transparent, rgba(255, 0, 255, 0.2), transparent)',
            }}
            animate={lineMotion.animate}
            transition={{
              ...lineMotion.transition,
...
/* ----------------------------------------------------------------------------
   2) CUSTOM HOOK: USE PARTICLES
   Manages mouse-based particle spawn and animation using requestAnimationFrame.
   Particles spawn randomly and fade out as they travel.
------------------------------------------------------------------------------*/
